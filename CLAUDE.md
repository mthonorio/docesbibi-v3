...
pnpm test     # vitest run — unit tests only (vitest.config.mts), no DB required
```
...
Vitest (`pnpm test`) is deliberately small — pure business-logic unit tests only (no jsdom, no React component tests, no live DB), e.g. `src/lib/products-visibility.test.ts`. `test-api.sh` is still a manual curl script for exercising the API routes end-to-end, not part of the automated suite.
...
Product images are attached, not pasted: `/admin/produtos`'s create/edit form uploads the file to `POST /api/products/upload-image` ([route.ts](src/app/api/products/upload-image/route.ts), staff-gated, 5MB cap, JPEG/PNG/WEBP/GIF only), which streams it to Cloudflare R2 via [src/lib/r2.ts](src/lib/r2.ts) (`uploadProductImage` — generates a random filename, never trusts the browser-supplied one) and returns the public URL (built with [src/lib/images.ts](src/lib/images.ts)'s `r2Image()`); the form just stores that URL string in `products.image`, same as always. Requires `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET_NAME`/`NEXT_PUBLIC_R2_PUBLIC_URL` at runtime, not just for the one-off `scripts/migrate-images-to-r2.mjs` migration — see [RAILWAY_DEPLOY.md](src/docs/RAILWAY_DEPLOY.md).
...
### Special categories (seasonal events)

`products.category` is (and stays) a plain string field with no table behind it — the fixed list (`personalizados`/`tradicionais`/`gourmet`/`easter`) lives in the frontend. **Special categories are a separate, independent concept**: a real table, `special_categories` (`sql/008_special_categories.sql`), representing a seasonal event (Páscoa, Dia das Mães, Black Friday...) with its own `enabled`/`start_date`/`end_date`. A product optionally belongs to at most one event via `products.special_category_id` (nullable FK, same N:1 shape as `category` — no join table). Managed at `/admin/eventos` (list + toggle) and `/admin/eventos/[id]` (edit + "Produtos do evento" add/remove), API at `/api/special-categories*` (staff-only — no public read endpoint; the SPEC only asks that linked products show/hide in the normal catalog, not a per-event landing page).

**The visibility rule lives in exactly one place**, [src/lib/products-visibility.ts](src/lib/products-visibility.ts): a product is publicly visible when `active = true` AND (it has no event, or its event is `enabled`). Disabling an event never touches `products.active` — reactivating the event alone brings its products back. This SQL fragment (`PUBLIC_PRODUCT_JOIN_SQL` + `PUBLIC_PRODUCT_VISIBILITY_SQL`) is reused by the only two public product-reading routes in the app, `GET /api/products` and `GET /api/products/[id]` (confirmed there's no search/sitemap/featured/related-products surface elsewhere to worry about) — direct access to a hidden product's URL 404s exactly like a nonexistent one, not a 403 that would confirm it exists. `includeInactive=true` (admin-only param) bypasses this whole check, same as it already bypassed `active`. The same rule is mirrored as a pure function, `isProductPubliclyVisible`, tested in `src/lib/products-visibility.test.ts` — keep the SQL and the function in sync if the rule ever changes.
